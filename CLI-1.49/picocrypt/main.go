...
func parse() int {
	...
	stdinPassword = flag.Bool(
		"stdin-password",
		false,
		"(ProjectVault) read password from stdin",
	)
	outputFile = flag.String(
		"o",
		"",
		"(ProjectVault) output file path",
	)
	flag.Parse()
...
func auth() int {
	if *stdinPassword {
		...
		if mode == "encrypt" {
			confirmp, err = reader.ReadBytes('\n')
			if err != nil && len(confirmp) == 0 {
				fmt.Println("Error reading password confirmation!")
				return 1
			}

			confirmp = bytes.TrimRight(confirmp, "\r\n")
